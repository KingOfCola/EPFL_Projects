...
elseif method=="product"
    neurons = randperm(48, 12);
    times = 1:4:20;
    feats = neurons + 48 * (times.' - 1);
    Data = misc.product(Data, reshape(feats,1,[]));
elseif method =="order"
    Data = [Data Data.^2];
end
...
figure()
for ax = 1:2
    [B, FitInfo] = lasso(xTrain, data.train.pos(:, ax), "Lambda", lambdas, "Alpha", alpha0);
    semilogx(lambdas, score.score(xValidation * B + FitInfo.Intercept, data.validation.pos(:, ax)))
    hold on
end
...
ylabel("MSE normalized")
axis([-inf inf 0 inf])
legend("Elastic nets posX", "Elastic nets posY", "Location","Southeast")
title("Elastic nets with alpha = " + alpha0)



%% Elastic nets multiple alpha
optLambdas = zeros(length(alphas), 2);
optScores = zeros(length(alphas), 2);
...
for i = 1:length(alphas)
    for ax = 1:2
        ...
        optScores(i, ax) = minScore;
        optLambdas(i, ax) = lambdas(minIndex);
    end
end

figure()
semilogx(alphas, optScores(:, 1))
hold on 
semilogx(alphas, optScores(:, 2))
xlabel("Alpha")
ylabel("MSE normalized")
axis([-inf inf 0 1])
legend("Elastic nets posX", "Elastic nets posY", "Location","Southeast")
title("Elastic nets with varying alpha")
...
%% Plot signal
[B, FitInfo] = lasso(xTrain, data.train.pos(:, ax), "Lambda", optLambdas(index), "Alpha", alphas(index));
figure()
plot(t, xValidation * B + FitInfo.Intercept)
hold on
plot(t, data.validation.posY)
...
[B, FitInfo] = lasso(xTrain, data.train.pos(:, ax), "Lambda", optLambdas(index), "Alpha", alphas(index));
score.score(xValidation * B + FitInfo.Intercept, data.validation.posY)

[B, FitInfo] = lasso(data.train.data, data.train.pos(:, ax), "Lambda", optLambdas(index), "Alpha", alphas(index));
score.score(data.validation.data * B + FitInfo.Intercept, data.validation.posY)
