...
methodsAll = [];
for i = 1:length(allGroceriesRavel)
    methodsAll = [methodsAll, allGroceriesRavel(i).discrimType + " " + allGroceriesRavel(i).method];
end

scoresAll = mean(scoresValidation,[1 2]);
figure()
boxplot(reshape(scoresAll, length(scoresAll), 1), methodsAll);
